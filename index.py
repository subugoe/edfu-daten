...
def addStellenTo(stellen, doc):
    ...
    for feld in ['seite_start', 'seite_stop', 'zeile_start', 'zeile_stop', 'band', 'stelle_anmerkung',
                 'stelle_unsicher', 'zerstoerung', 'freigegeben']:
        if not feld in doc:
            doc[feld] = []
            if feld == 'band':
                doc['bandseite'] = []
            ...
            if feld == 'band':
                ...
                if intToRoman.has_key(bandNummer):
                    ...
                    if stelle['seite_stop'] == stelle['seite_start']:
                        ...
                        bandSeiteZeile += ("%02d" % (stelle['zeile_start'])) + ' - ' + (
                            "%03d" % (stelle['seite_stop'])) + ', ' + ("%02d" % (stelle['zeile_stop']))
...
def submitDocs(docs, name):
    ...
    index = solr.Solr('http://adw-dev.tc.sub.uni-goettingen.de/solr/edfu-dev')
    index.add_many(docs)
    index.commit()
...
for (uid, seite_start, seite_stop, zeile_start, zeile_stop, anmerkung, stop_unsicher, zerstoerung, band, freigegeben,
     tempel) in cursor:
...
# WORT
query = (
    "SELECT uid,id,transliteration,weiteres,uebersetzung,anmerkung,hieroglyph,lemma,wb_berlin_uid FROM tx_edfu_domain_model_wort")
cursor.execute(query)
