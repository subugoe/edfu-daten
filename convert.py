...
def szSplit(s):
    ...
    try:
        ...
        print "Fehler bei der Auftrennung von " + s
...
bandDict = {
1: {'uid': 1, 'nummer': 1, 'freigegeben': False, 'literatur': u'Chassinat, Émile; Le Temple d’Edfou I, 1892.',
    'tempel_uid': 0},
2: {'uid': 2, 'nummer': 2, 'freigegeben': False, 'literatur': u'Chassinat, Émile; Le Temple d’Edfou II, 1897.',
    'tempel_uid': 0},
3: {'uid': 3, 'nummer': 3, 'freigegeben': False, 'literatur': u'Chassinat, Émile; Le Temple d’Edfou III, 1928.',
    'tempel_uid': 0},
4: {'uid': 4, 'nummer': 4, 'freigegeben': False, 'literatur': u'Chassinat, Émile; Le Temple d’Edfou IV, 1929.',
    'tempel_uid': 0},
5: {'uid': 5, 'nummer': 5, 'freigegeben': False, 'literatur': u'Chassinat, Émile; Le Temple d’Edfou V, 1930.',
    'tempel_uid': 0},
6: {'uid': 6, 'nummer': 6, 'freigegeben': False, 'literatur': u'Chassinat, Émile; Le Temple d’Edfou VI, 1931.',
    'tempel_uid': 0},
7: {'uid': 7, 'nummer': 7, 'freigegeben': True, 'literatur': u'Chassinat, Émile; Le Temple d’Edfou VII, 1932.',
    'tempel_uid': 0},
8: {'uid': 8, 'nummer': 8, 'freigegeben': True, 'literatur': u'Chassinat, Émile; Le Temple d’Edfou VIII, 1933.',
    'tempel_uid': 0}
}
band = []
...
def finishCollection(PRIMARY):
    ...
    for c in photo_collection:
        if c['items'] == collection['items'] and c['kommentar'] == collection['kommentar'] and c['klammern'] == \
            collection['klammern'] and c['stern'] == collection['stern']:
            collection = c
            break
...
for (PRIMARY, TEXTMITSUF, BAND, SEITEZEILE, TEXTOHNESU, TEXTDEUTSC, TEXTTYP, Photo, SzenenID, SekLit) in cursor:
    ...
    if myFormular['uebersetzung'] != TEXTDEUTSC:
        sys.stdout.write("\t".join(["FL", str(PRIMARY), "INFO", u"Übersetzung String verändert", TEXTDEUTSC, myFormular['uebersetzung']]))

    if re101.search(myFormular['uebersetzung']) or re102.search(myFormular['uebersetzung']):
        sys.stdout.write("\t".join(["FL", str(PRIMARY), "WARNUNG", u"Vermutlich kaputte Akzente", myFormular['uebersetzung']]))
    ...
    if Photo == 'D05_5503, D05_5504, D05_5509, D05_5510, D05_5511, D05_5512: D05_5513, D05_5514, ( 2982, 2983, 2984, 2985 )*':
        ...
        '( 2438, 2439, 2440, 2441, 2442, 2443, 2444, 2445, 2446, 2447, 2448, 2449, 2450, 2451 (E. VIII, 96, 3 - 99, 3))*') != -1:
        # 9741-9773
    ...
    if Photo != originalPhoto:
        print "\t".join(["FL", str(PRIMARY), "INFO", u"Photo String veraendert", originalPhoto, Photo])
    ...
    if szOriginal != SEITEZEILE:
        print "\t".join(["FL", str(PRIMARY), "INFO", u"Aenderung SEITEZEILE", szOriginal, SEITEZEILE])
    ...
    for teil in teile:
        if len(teil.strip()) > 0:
            ...
            if not m3:
                ...
                if myStelle['zeile_start'] > 30:
                    print "\t".join(
                        ["OL", str(PRIMARY), "FEHLER", "zeile_start > 30: " + str(myStelle['zeile_start']), teil])
                if myStelle['zeile_stop'] > 30:
                    print "\t".join(
                        ["OL", str(PRIMARY), "FEHLER", "zeile_stop > 30: " + str(myStelle['zeile_stop']), teil])
...
for o in ort[:]:
    ...
    if translit == previousTranslit and o['ort'] == previousO['ort'] and o['lokalisation'] == previousO['lokalisation']:
        ...
        print "\t".join(
            ["OL", str(PRIMARY), "INFO", str(o['uid']) + u" Duplikat von " + str(previousO['uid']) + u": mergen",
             o['transliteration'], previousO['transliteration']])
...
for (PRIMARY, NAME, ORT, EPON, BEZ, FKT, BND, SEITEZEILE, ANM) in cursor:
    ...
    if len(szs) == 1 and len(szs[0]) > 1:
        ...
        if startSeite > 0 and band > 0:
            ...
            print "\t".join(
                ["GL", str(PRIMARY), "FEHLER", u"startSeite oder Band nicht ermittelbar: Datensatz verwerfen", sz])
    else:
        print "\t".join(
            ["GL", str(PRIMARY), "FEHLER", u"nicht genau eine Stelle in SEITEZEILE: Datensatz verwerfen", SEITEZEILE])
...
# Tabelle WL
query = (
"SELECT `PRIMARY`, Transliteration, Deutsch, IDS, Weiteres, BelegstellenEdfu, BelegstellenWb, Anmerkungen FROM WL")
cursor.execute(query)
...
# In MySQL einfügen
db = mysql.connector.connect(user='edfu', host='127.0.0.1', database='edfu', password='edfu')
cursor = db.cursor()
